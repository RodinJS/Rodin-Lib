...
System.register(['rodin/core', './Teleport.js'], function (_export, _context) {
    ...
    var RODIN, Teleport, box, lights, targetObj, pointOBJ, teleport, qaq;
    return {
        ...
        execute: function () {
            ...
            targetObj = new RODIN.Sculpt(new THREE.Mesh(new THREE.IcosahedronGeometry(1, 4), new THREE.MeshPhongMaterial({
                color: 0x156289,
            ...
            pointOBJ = new RODIN.Box(0.1, new THREE.MeshNormalMaterial({ wireframe: false }));
            teleport = new Teleport(box, undefined, undefined, [targetObj]);

            targetObj.on(RODIN.CONST.GAMEPAD_BUTTON_DOWN, function (e) {
                console.log("down");
                e.stopPropagation();
            });
            targetObj.on(RODIN.CONST.GAMEPAD_BUTTON_UP, function (e) {
                console.log("up", e);
            });

            targetObj.on(RODIN.CONST.UPDATE, function (e) {
                targetObj.rotation.y += 0.005;
                targetObj.rotation.x += 0.001;
                targetObj.rotation.z += 0.007;
                targetObj._threeObject.updateMatrixWorld();
                var inter = teleport.getIntersection();
                if (inter.length > 1) {
                    //console.log(inter[0])
            ...
            targetObj.on(RODIN.CONST.GAMEPAD_HOVER, function (e) {
                console.log("hover");
            });
            targetObj.on(RODIN.CONST.GAMEPAD_HOVER_OUT, function (e) {
                console.log("shoqer");
            });
            RODIN.GamePad.mouse.on(RODIN.CONST.GAMEPAD_HOVER, function (e) {
                //console.log(e.target.intersected[0]);
                //if (e.target.intersected[0])console.log(e.target.intersected[0].distance);
            });
            RODIN.GamePad.mouse.on(RODIN.CONST.UPDATE, function (e) {
            ...
            RODIN.GamePad.mouse.on(RODIN.CONST.GAMEPAD_HOVER_OUT, function (e) {});
            RODIN.GamePad.mouse.on(RODIN.CONST.GAMEPAD_BUTTON_UP, function (e) {});
            qaq = 0;


            box.on(RODIN.CONST.UPDATE, function (e) {
                box.rotation.y = Math.sin(qaq) / 5 - 0.2;
                box.position.x = Math.sin(qaq) / 5 - 0.5;
                qaq += 0.1;
            });

            RODIN.GamePad.viveRight.on(RODIN.CONST.GAMEPAD_BUTTON_DOWN, function (e) {
            ...
            RODIN.GamePad.mouse.on(RODIN.CONST.GAMEPAD_BUTTON_DOWN, function (e) {
                console.log("down", e.button[0].keyCode);
            });
            RODIN.GamePad.viveRight.on(RODIN.CONST.GAMEPAD_BUTTON_UP, function (e) {
                console.log("up", e);
            });
