...
let targetObj = new RODIN.Sculpt(
    new THREE.Mesh(
        ...
        new THREE.PlaneGeometry(4, 4, 0,0),
...
const teleport = new Teleport(box);

/*
 const p = new RODIN.Plane(100, new THREE.MeshBasicMaterial({color: 0xbbbbbb}));
...
/*
 ...
 const bbcad = new RODIN.Box(1, .4, 1, new THREE.MeshBasicMaterial({color: 0xFF0000}));
 bbcad.position.set(-10, .2, -5);
 RODIN.Scene.add(bbcad);
 bbcad.on(RODIN.CONST.GAMEPAD_BUTTON_UP, (evt) => {
 RODIN.Avatar.active.position = evt.target.position;
 // console.log('---------------------------');
 // console.log(evt.target.position.x);
 // console.log(evt.target.position.y);
 // console.log(evt.target.position.z);
 });

 const bbc = new RODIN.Box(1, .4, 1, new THREE.MeshBasicMaterial({color: 0x00FF00}));
 bbc.position.set(-10, .2, 5);
 RODIN.Scene.add(bbc);
 bbc.on(RODIN.CONST.GAMEPAD_BUTTON_UP, (evt) => {
 RODIN.Avatar.active.position = evt.target.position;
 // console.log('---------------------------');
 // console.log(evt.target.position.x);
 // console.log(evt.target.position.y);
 ...
 const bbca = new RODIN.Box(1, .4, 1, new THREE.MeshBasicMaterial({color: 0x0000FF}));
 bbca.position.set(10, .2, -5);
 RODIN.Scene.add(bbca);
 bbca.on(RODIN.CONST.GAMEPAD_BUTTON_UP, (evt) => {
 RODIN.Avatar.active.position = evt.target.position;
 // console.log('---------------------------');
 // console.log(evt.target.position.x);
 // console.log(evt.target.position.y);
