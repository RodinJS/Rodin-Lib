...
export class Text3D extends Sculpt {
    ...
    constructor(...args) {
        ...
        if (!args.material) {
            args.material = new THREE.MeshBasicMaterial({color: args.color})
        }
        ...
        if(!this._font.substring(this._font.lastIndexOf('.')+1).startsWith("json")){
            Loader.loadFont(this._font, (rawFont)=>{
                Text3D.fonts[this._font] = threeFontLoader.parse(
                    JSON.parse(
                        convert(rawFont)
                    )
                );
    ...
    drawFromJSON(){
        if (!Text3D.fonts.hasOwnProperty(this._font)) {
            threeFontLoader.load(this._font, (font)=> {
                this.draw(font);
                Text3D.fonts[this._font] = font;
            });
        } else {
            this.draw(Text3D.fonts[this._font]);
        }
